...
class DBHelper:
    def __init__(self):
        self.con = sqlConnector.connect(host="localhost", user="root", passwd="XXXXXXXX", database="algo", port="3306")
        self.conTick = sqlConnector.connect(host="localhost", user="root", passwd="xxxxxxxxx", database="algo", port="3306")
        self.conTick2 = sqlConnector.connect(host="localhost", user="python", passwd="Axxxxxxyyy", database="algo", port="3306")
    ...
    def GetNiftyOneMinuteHistoricalData(self, start_date, end_date):
        query = "SELECT date, open, high,low,close, '0' as volume FROM nifty_historical_data_01min where date between '{}' and '{}' order by date".format(start_date, end_date)
        df = pd.read_sql(query, con=self.con)
        return df
    ...
    def insert_fake_zeroda_orders(self, trading_symbol, qty, exchange, trans_type, timestamp, product, order_type, price, stop_loss_trigger):
    ...
    def get_fake_zeroda_orderid(self):
    ...
    def update_SLtrigger_price_fake_zeroda(self, stop_loss_trigger, order_id):
    ...
    def GetTradeByDate_fake_zeroda(self, startdate, enddate):
        query = "Select * from fake_zeroda_trades where timestamp between '{}' and '{}'".format(startdate, enddate)
        df = pd.read_sql(query, con=self.con)
        return df

    def GetTradeByOrderID_fake_zeroda(self, order_id):
    ...
    def update_order_status_fake_zeroda(self, status, order_id):
    ...
    def GetPositions_fake_zeroda(self):
    ...
    def Get_NiftyFut_LTP_fake_zeroda(self, cur_date, cur_time):
        query = "SELECT LTP FROM Tick where `Date` = '{}' and Time <= '{}' order by `index` desc LIMIT 1;".format(cur_date, cur_time)
        df = pd.read_sql(query, con=self.conTick)
        return df
    
    def GetTickData_fake_zeroda(self, ticker, cur_date, start_time, cur_time):
        query = "SELECT Date, Time, LTP FROM Tick where Ticker = '{}' and Date = '{}' and Time between '{}' and '{}';".format(ticker, cur_date, start_time, cur_time)
        #print(query)
        df = pd.read_sql(query, con=self.conTick)
        return df
    ...
    def InsertTrade_Fake_zeroda(self, order_id, exchange, tradingsymbol, average_price, quantity, transaction_type, timestamp):
    ...
    def getPositions_Fake_zeroda(self, tradingsymbol):
        query = "SELECT * FROM fake_zeroda_positions where tradingsymbol = '{}'".format(tradingsymbol)
        df = pd.read_sql(query, con=self.con)
        return df
    
    def updatePosition_fake_zeroda(self, quantity, average_price, last_price, m2m, tradingsymbol):
    ...
    def updateCompleteOrder_fake_zeroda(self, orderid, price):
        query = "update fake_zeroda_orders set status = 'COMPLETE', price = '{}' where order_id = '{}'".format(price, orderid)
        cur = self.con.cursor()
        cur.execute(query)
        self.con.commit()
    
    def getOrderHistory_fake_zeroda(self, orderid):
    ...
    def InsertOHLCData(self, date, open, high, low, close):
        query = "INSERT into Algo.NIFTY_OHLC_Daily(date, open, high, low, close) Values('{}','{}','{}','{}','{}')".format(date, open, high, low, close)
        cur = self.con.cursor()
        cur.execute(query)
        self.con.commit()
